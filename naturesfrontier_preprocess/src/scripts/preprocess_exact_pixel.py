...
from wbnci.preprocessing_exact_pixel import (create_pixel_grid, aggregate_pixel_values,
                                 build_pixel_score_table, join_table_to_grid, read_country_list)
...
def main(config_file):
    ...
    for country in countries:
        ...
        # Modified: Use create_pixel_grid instead of create_sdus
        create_pixel_grid_task = tg_create_pixel_grid(tg, country_root, args, mask_base_raster_task)
        ...
        # Modified: Use aggregate_pixel_values instead of aggregate_results
        create_results_aggregation_tasks = tg_aggregate_pixel_results(
            tg, country_root, create_pixel_grid_task, create_scenario_tasks, create_scenario_evaluation_tasks)
...
# Modified: Create pixel grid instead of SDUs
def tg_create_pixel_grid(tg, country_root, args, mask_base_raster_task):
    c = os.path.basename(country_root)
    output_file = os.path.join(country_root, "pixel_map", "pixel_grid.tif")
    make_pixel_task = tg.add_task(
        func=create_pixel_grid,
        args=[country_root, args],
        task_name=f"create_pixel_grid_{c}",
        target_path_list=[output_file],
        dependent_task_list=[mask_base_raster_task]
    )
    return make_pixel_task
...
# Modified: Aggregate results to pixel level instead of SDU level
def tg_aggregate_pixel_results(tg, country_root, pixel_grid_task, scenario_tasks, evaluation_tasks):
    """
    Create the Value Tables at pixel level
    """
    ...
    pixel_grid_raster = os.path.join(country_root, "pixel_map", "pixel_grid.tif")
    ...
    for t in transition_names:
        ...
        task_deps = [evaluation_tasks[(t, et)] for et in eval_task_names] + [pixel_grid_task, scenario_tasks["sustainable_current"]]
        tasks[t] = tg.add_task(
            func=_score_table_for_pixel,
            args=[t, services, model_results_folder, pixel_grid_raster,
                  mask_raster, output_folder, transition_names],
            task_name=f"{country_name}_aggregate_to_pixels_{t}",
            hash_algorithm="md5",
            target_path_list=[target_file],
...
def _score_table_for_pixel(transition, services, model_results_folder, pixel_grid_raster, 
                           mask_raster, output_folder, transition_names):
    raster_lookup = {
        s: os.path.join(model_results_folder, f'{transition}_{s}.tif') for s in services
    }
    pixel_values = aggregate_pixel_values(pixel_grid_raster, 'SDUID', mask_raster, raster_lookup)
    table_file = os.path.join(output_folder, f'{transition}.csv')
    build_pixel_score_table('SDUID', transition_names, transition, pixel_values, None, table_file)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise Exception("Usage: python preprocess_exact_pixel.py [config file path]")
