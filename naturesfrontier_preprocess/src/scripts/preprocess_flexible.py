...
from wbnci.preprocessing_flexible import (create_spatial_decision_units, aggregate_values,
                                 build_score_table, join_table_to_grid, read_country_list)
...
def main(config_file):
    ...
    # Get spatial unit type from config (default to hexagon)
    ...
    for country in countries:
        ...
        # Create spatial decision units based on selected method
        create_sdu_task = tg_create_spatial_decision_units(
            tg, country_root, args, mask_base_raster_task, spatial_unit_type)
        ...
        # Aggregate results using the selected method
        create_results_aggregation_tasks = tg_aggregate_results(
            tg, country_root, create_sdu_task, create_scenario_tasks, 
            create_scenario_evaluation_tasks, spatial_unit_type)
...
def tg_create_spatial_decision_units(tg, country_root, args, mask_base_raster_task, spatial_unit_type):
    ...
    make_sdu_task = tg.add_task(
        func=create_spatial_decision_units,
...
def tg_aggregate_results(
        tg, country_root, sdu_task, scenario_tasks, evaluation_tasks, spatial_unit_type):
    """
    Create the Value Tables using the appropriate spatial decision unit type
    """
    ...
    # Set paths based on spatial unit type
    if spatial_unit_type == "pixel":
        spatial_unit_path = os.path.join(country_root, "pixel_map", "pixel_grid.tif")
    elif spatial_unit_type == "coarser_grid":
        spatial_unit_path = os.path.join(country_root, "coarser_grid_map", "coarser_grid_detailed.tif")
    else:  # hexagon
        spatial_unit_path = os.path.join(country_root, "sdu_map", "sdu.shp")
    ...
    for t in transition_names:
        ...
        tasks[t] = tg.add_task(
            ...
            args=[t, services, model_results_folder, spatial_unit_path,
                  mask_raster, output_folder, transition_names, spatial_unit_type],
            task_name=f"{country_name}_aggregate_to_spatial_units_{t}",
            hash_algorithm="md5",
            target_path_list=[target_file],
...
def _score_table_for_scenario(
        transition, services, model_results_folder, spatial_unit_path, 
        mask_raster, output_folder, transition_names, spatial_unit_type):
    """
    Create a score table for a specific scenario using the appropriate spatial unit type
    """
    # Create lookup for value rasters
    raster_lookup = {
        s: os.path.join(model_results_folder, f'{transition}_{s}.tif') for s in services
    }
    
    # Aggregate values using the selected method
    aggregated_values = aggregate_values(
        spatial_unit_path, 'SDUID', mask_raster, raster_lookup, spatial_unit_type)
    
    # Build score table
    table_file = os.path.join(output_folder, f'{transition}.csv')
    build_score_table(
        'SDUID', transition_names, transition, aggregated_values, 
        None, table_file, spatial_unit_type)
    
    # Process the resulting CSV to add derived columns
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise Exception("Usage: python preprocess_flexible.py [config file path]")
