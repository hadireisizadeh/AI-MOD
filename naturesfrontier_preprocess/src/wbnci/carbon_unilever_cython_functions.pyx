...
import cython
cimport cython
import numpy as np  # NOTE, both imports are required. cimport adds extra information to the pyd while the import actually defines numpy
cimport numpy as np
...
def write_carbon_table_to_array(
        ...
        ndarray[np.float32_t, ndim=2] pixel_area not None,
    ...
    for cr in range(n_rows):
        for cc in range(n_cols):
            ...
            if c_carbon_zone > 0 and c_lulc_class > 0:
                try:
                    output_carbon[cr, cc] = pixel_area[cr, cc] * lookup_table[row_names[c_carbon_zone], col_names[c_lulc_class]]
                except:
                    pass
